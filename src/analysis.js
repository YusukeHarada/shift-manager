...
// 疲労度の重み。夜勤・当直の長時間拘束を重く、休みだけを回復として引く。
// 日勤帯は始業が早いほど楽という利用者の実感に合わせ、始業時刻の順に並べている
// （遅番 10:15 → 日勤 8:45 → 早番 7:30 → 早番1 7:00）
//
// 明け休みは 0。9:30 まで働いた日なので休養にはならず、かといって負荷も積まない。
// 以前は休みと同じ向きの -0.5 を充てていたが、夜勤明けで回復するという扱いになり
// 夜勤が続く月が軽く出ていた（利用者の指摘）
...
// 夜勤と明け休みだけが続く最も重い並びを指数 100 の手前に置くためのスケール。
// 実測では定常値 6.9（指数 76）で、通常のローテーション（日・早・遅・夜・明・休）は
// 平均 38 に収まる。重みを変えたらこの2つを測り直すこと
