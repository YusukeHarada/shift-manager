...
**夜勤と明け休みは同じローズ系にする。** 夜勤の翌日に必ず明け休みが来るため、並んだときにペアと分かるほうがよい（利用者の要望）。区別は彩度でつける — 夜勤は濃いローズ、明け休みは彩度を落として「休み」であることを示す。**夜勤は回数が多く一番読めてほしいため、他と重ならないローズを充てている**（以前は紫で見づらいという指摘があった）。
